...
{
    fn ty() -> Option<Type> {
        Some(ty::list(<T as Typed>::ty()))
    }
}

impl<T> Typed for std::marker::PhantomData<T> {
    fn ty() -> Option<Type> {
        Some(Type::Unit)
    }
}

impl Typed for bytes::Bytes {
    fn ty() -> Option<Type> {
        Some(Type::Raw)
    }
}

// Does not require T: Typed.
impl<T> Typed for [T; 0] {
    fn ty() -> Option<Type> {
        Some(Type::Unit)
    }
}
...
macro_rules! impl_array_valuable {
    ($len:literal) => {
        seq_macro::seq!(N in 1..$len {
            #(
                impl<T> $crate::Typed for [T; N]
                where
                    T: $crate::Typed,
                {
                    fn ty() -> Option<$crate::Type> {
                        let value_ty = <T as Typed>::ty();
                        let tuple = $crate::ty::Tuple::Tuple(
                            vec![value_ty; N]
                        );
                        Some($crate::ty::Type::Tuple(tuple))
                    }
                }
            )*
        });
...
impl<K, V> Typed for std::collections::HashMap<K, V>
where
    K: Typed,
    V: Typed,
{
